...
function coverageSnapshot(map, useAdjusted, open="09:30", close="19:15"){
  const covOpen=[0,0,0,0,0,0,0], covClose=[0,0,0,0,0,0,0];
  const openM=minStrLocal(open), closeM=minStrLocal(close)-1;
...
function buildSnapshotTable(details){
  ...
  const coverage = coverageSnapshot(map, useAdjusted);
  const hours = weeklyHoursSnapshot(map, useAdjusted);
  const openMinute = minStrLocal('09:30');
  const closeMinute = minStrLocal('19:15') - 1;
