#!/usr/bin/env python3
"""Idempotent seeding: n'ajoute que ce qui manque.

Usage:
  PYTHONPATH=. python3 scripts/seed_base_safe.py
"""
from backend import create_app


def add_shift(emp_name, weekday, start, end, lstart=None, lend=None):
    from backend.models import Employee, BaseShift, db
    emp = Employee.query.filter_by(name=emp_name).first()
    if not emp:
        print(f"Employee {emp_name} not found, skipping shift {start}-{end} on {weekday}")
        return
    exists = BaseShift.query.filter_by(employee_id=emp.id, weekday=weekday, start_time=start, end_time=end).first()
    if exists:
        return
    s = BaseShift(employee_id=emp.id, weekday=weekday, start_time=start, end_time=end, lunch_start=lstart, lunch_end=lend)
    db.session.add(s)


def main():
    from backend.models import db, Employee, OpeningHours
    ...
    with app.app_context():
        ...
        for name, ctype, rules in emps:
            existing = Employee.query.filter_by(name=name).first()
            if existing:
                continue
            db.session.add(Employee(name=name, contract_type=ctype, special_rules=rules))
        db.session.commit()

        # Opening hours for 0..6
        for wd in range(0, 7):
        ...
        db.session.commit()
        print("Safe seed applied: employees/openings/shifts added where missing.")
