async function fetchSnapshots(){
  ...
  let html = '<table><tr><th>ID</th><th>Lundi</th><th>Label</th><th>Status</th><th>Actions</th></tr>';
  for(const s of data){
    html += `<tr><td>${s.id}</td><td>${s.monday}</td><td>${s.label||''}</td><td>${s.status||''}</td><td>`+
      `<button data-id="${s.id}" class="view">Voir</button> `+
      `<button data-id="${s.id}" class="restore">Restaurer</button> `+
      `<button data-id="${s.id}" class="download">Télécharger</button> `+
  ...
  for(const b of document.querySelectorAll('button.view')){
    b.onclick = async (e)=>{
      ...
      // open rich preview page
      window.location.href = `/snapshots/${id}/view`;
    }
  }
  for(const b of document.querySelectorAll('button.restore')){
    b.onclick = async (e)=>{
      const id = e.target.dataset.id;
      if(!confirm('Restaurer cette archive ?')) return;
  ...
  for(const b of document.querySelectorAll('button.del')){
    b.onclick = async (e)=>{
      ...
      if(r.status===204) { alert('Supprimé'); fetchSnapshots(); } else alert('Erreur');
    }
  }
  for(const b of document.querySelectorAll('button.download')){
    b.onclick = async (e)=>{
      const id = e.target.dataset.id;
      const url = '/api/snapshots/'+id+'/download';
      // trigger download
...
document.getElementById('refresh').addEventListener('click', fetchSnapshots);
window.addEventListener('load', fetchSnapshots);
