...
from .models import db, Employee, BaseShift, AdjustedShift, Absence, OpeningHours, RuleViolation
...
def recommended_week_minutes(emp: Employee) -> int:
    ...
    if base_hours is None and digits:
        try:
            base_hours = float(int(digits))
        except ValueError:
            base_hours = None
...
def overtime_allowance_minutes(emp: Employee) -> int:
    ...
    hours = None
    if getattr(emp, "max_week_hours", None) is not None:
        hours = int(emp.max_week_hours)
    if hours is None and digits:
        try:
            hours = int(digits)
        except ValueError:
            hours = None
...
def generate_adjusted_week(monday: date, sunday_open: bool = False) -> None:
    ...
    for idx, day in enumerate(days):
        if idx == 6 and not sunday_open:
            continue
        oh = opening_hours.get(idx)
        open_time = hhmm(oh.open_time) if oh and oh.open_time else "09:30"
        close_time = hhmm(oh.close_time) if oh and oh.close_time else "19:30"
